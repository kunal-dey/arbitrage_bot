...
from utils.training_modules.data_preparation import training_data
...
def create_model(hl=2, hn=40, dropout=False, input_dim=None, rate=0.3):
    ...
    for layer in range(hl):
        model.add(keras.layers.Dense(units=hn, activation='relu'))
    if dropout:
        model.add(keras.layers.Dropout(rate=rate, seed=100))
...
def train_model(stock_list):
    ...
    data_df = training_data(stock_list)

    logger.info(f"size: {data_df}")

    train, train_s, test, test_s = split_data(split_ratio=0.99, data_df=data_df)
    ...
    model.save(getcwd() + "/temp/DNN_model.h5")
